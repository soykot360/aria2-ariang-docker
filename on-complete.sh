...
UPLOAD_FILE() {
    ...
    while [ ${RETRY} -le ${RETRY_NUM} ]; do
		...
		if [ -n "${RCLONE_DEST_FOLDER_2}" ]; then
...
if [ "${TOP_PATH}" = "${FILE_PATH}" ] && [ $2 -eq 1 ]; then # 普通单文件下载，移动文件到设定的网盘文件夹。
    ...
    REMOTE_PATH="${RCLONE_DEST_FOLDER}/"
    REMOTE_PATH_2="${RCLONE_DEST_FOLDER_2}/"
    UPLOAD
    exit 0
elif [ "${TOP_PATH}" != "${FILE_PATH}" ] && [ $2 -gt 1 ]; then # BT下载（文件夹内文件数大于1），移动整个文件夹到设定的网盘文件夹。
    UPLOAD_PATH="${TOP_PATH}"
    REMOTE_PATH="${RCLONE_DEST_FOLDER}/${REMOVE_DOWNLOAD_PATH%%/*}"
	REMOTE_PATH_2="${RCLONE_DEST_FOLDER_2}/${REMOVE_DOWNLOAD_PATH%%/*}"
    CLEAN_UP
    UPLOAD
    exit 0
